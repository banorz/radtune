...
- **Two tabs, two container windows.** `g_pageTuning` / `g_pageLive` are child windows at the same rect; exactly one is visible. The first attempt overlaid the Live page on its siblings — overlapping sibling windows repaint over each other, so the tuning form showed through. Controls live on a page, so `PageProc` forwards `WM_COMMAND`/`WM_DRAWITEM` up to `WndProc`, which keeps every handler in one place (forget to add a case there and the click is silently dropped — that is exactly how Refresh once did nothing).
