...
2. **GUI** — done: `RadTuneGUI.exe`, a native Win32 frontend (see [architecture.md](architecture.md#gui--guiradtuneguicpp)). It builds `-set` / `-load` commands and drives `-schedule`, shelling out to `RadTune.exe`. Chose plain Win32 over Dear ImGui (zero extra dependencies, builds with the stock toolchain). Not yet done here: persisting the form to a `radtune.json` — the natural next step, and the shared state the watchdog will consume.
