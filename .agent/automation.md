...
2. **GUI** — done: `RadTuneGUI.exe`, a native Win32 frontend (see [architecture.md](architecture.md#gui--guiradtuneguicpp)). Builds `-set` / `-load` / `-get` commands and drives `-schedule`, shelling out to `RadTune.exe`. Chose plain Win32 over Dear ImGui (zero extra dependencies, builds with the stock toolchain). It can read current values back (`-get`) and remembers the form in the registry (`HKCU\Software\RadTune`). Still open: exporting the desired state to a shared `radtune.json` that the watchdog can consume (the registry store is GUI-local).
